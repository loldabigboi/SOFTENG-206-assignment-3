 ...
 * could have helped in this regard, but given that we were given the option to not use it, I don't think this is a valid counter-point.
 */

public class WikiCreationMenu extends Stage {
	
	private static final String validCharacters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456789 (),&-_";
	...
	public WikiCreationMenu(MainMenu mainMenu, Stage parentStage, String wikiTerm, String wikiText) {
		...
		saveButton.setOnAction((e) -> {
			...
			if (fileExists == 0) {
				...
				if (clickedButton != ButtonType.CONFIRM) {
					return;
				}
		...
		selectImagesButton.setOnAction((e) -> {
			...
			if (fileExists == 0) {  // creation with same name already exists
				...
				ButtonType buttonClicked = alertDialog.showAndReturn();
				if (buttonClicked != ButtonType.CONFIRM) {
					return;
				}
